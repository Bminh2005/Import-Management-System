...
public class RequestRepository {
    ...
    private Request loadRequest(Connection conn, long id) throws SQLException {
        String sql = "SELECT r.id, r.created_at::date AS created_date, r.status, "
                ...
                + "COALESCE(NULLIF(("
                + "  SELECT string_agg(DISTINCT s.site_name, ', ' ORDER BY s.site_name) "
                + "  FROM \"Order\" o "
                + "  LEFT JOIN \"Site\" s ON s.id = o.site_id "
                + "  WHERE o.request_id = r.id AND COALESCE(s.site_name, '') <> ''"
                + "), ''), "
                + "  CASE r.status::text "
                + "    WHEN 'PROCESSED' THEN 'Đặt hàng Quốc tế' "
                + "    WHEN 'PROCESSING' THEN 'Bộ phận Mua hàng' "
                + "    ELSE 'Chưa phân công' "
                + "  END) AS assigned_to "
    ...
    /** Đọc mặt hàng thuộc một đơn hàng (bảng OrderDetail). */
    public List<OrderItem> findOrderItems(String orderCode) {
        ...
        try (Connection conn = DatabaseManager.getConnection();
            ...
            throw new RuntimeException("Lỗi đọc mặt hàng đơn hàng " + orderCode, e);
        }
        return result;
    }

    /** Đọc items của 1 yêu cầu (dùng khi đơn chưa có dòng OrderDetail). */
