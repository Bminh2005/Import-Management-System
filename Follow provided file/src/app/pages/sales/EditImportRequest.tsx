...
import { ArrowLeft, Save, Trash2, Plus, X, XCircle, CreditCard, Eye } from 'lucide-react';
...
const mockRequestData = {
  ...
  handler: 'Trần Thị B - Bộ phận Đặt hàng Quốc tế',
  items: [
    {
      id: '1',
      code: 'MH001',
...
export function EditImportRequest() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [request, setRequest] = useState(mockRequestData);
  const [items, setItems] = useState(mockRequestData.items);
  ...
  const [paymentStep, setPaymentStep] = useState<'method' | 'details' | 'confirm'>(
    'method'
  );
  ...
  const handleSave = () => {
    ...
    setHasChanges(false);
    navigate(`/sales/requests/${id}`);
  };

  const handleCancel = () => {
    if (hasChanges) {
      if (confirm('Bạn có những thay đổi chưa lưu. Bạn có chắc muốn hủy?')) {
        navigate(`/sales/requests/${id}`);
      }
    } else {
      navigate(`/sales/requests/${id}`);
  ...
  const handleCancelRequest = () => {
    ...
    alert(`Đã hủy yêu cầu ${request.code} thành công!`);
    setShowCancelRequestModal(false);
    navigate('/sales/requests');
  };

  const getItemStatusColor = (status: RequestItem['status']) => {
    const colors = {
  ...
  const getItemStatusLabel = (status: RequestItem['status']) => {
    const labels = {
      pending: 'Chờ duyệt',
      approved: 'Đã duyệt',
      rejected: 'Từ chối',
  ...
  const handlePayOrder = () => {
    alert(`Đã xác nhận thanh toán đơn hàng ${selectedOrder?.code} thành công!`);
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-semibold text-[var(--text-primary)]">
            Chỉnh sửa Yêu cầu: {request.code}
          </h1>
          ...
          <table className="w-full">
            <thead className="bg-[var(--muted)]">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase">Ngày nhận</th>
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-[var(--muted)] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getItemStatusColor(
                        item.status
                      )}`}
                    >
      ...
      <div className="flex justify-end gap-3">
        ...
        <button
          onClick={handleSave}
          disabled={!hasChanges}
          className="flex items-center gap-2 px-6 py-2.5 bg-[var(--primary)] text-white rounded-lg hover:bg-blue-700 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        >
          ...
          >
            <div className="p-6 border-b border-[var(--border)]">
              <h3 className="text-xl font-semibold text-red-700">Hủy yêu cầu nhập hàng</h3>
            </div>

            <div className="p-6 space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <p className="text-sm text-red-800">
                  <strong>Cảnh báo:</strong> Hành động này sẽ hủy toàn bộ yêu cầu nhập hàng{' '}
                  <strong>{request.code}</strong>. Tất cả các mặt hàng sẽ bị hủy và không thể khôi phục.
                </p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-[var(--text-secondary)] uppercase mb-2">
                  Lý do hủy yêu cầu <span className="text-red-600">*</span>
                </label>
                <textarea
                  value={cancelReason}
                  onChange={(e) => setCancelReason(e.target.value)}
                  placeholder="Nhập lý do hủy yêu cầu nhập hàng..."
                  rows={4}
                  className="w-full px-4 py-2 border border-[var(--border)] rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 resize-none"
                />
              </div>
            </div>
            ...
            <div className="p-6 border-t border-[var(--border)] flex justify-end gap-3">
              <button
                ...
                className="px-6 py-3 border border-[var(--border)] rounded-lg hover:bg-[var(--muted)] transition-colors font-medium"
              >
                Quay lại
              </button>
              <button
